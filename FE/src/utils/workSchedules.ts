...
export const fetchAvailableScheduleDoctor = async (): Promise<WorkSchedule[]> => {
  try {
    const response = await Axios(SummaryApi.schedules.getAvailableScheduleDoctor);
...
export const registerScheduleDoctor = async (id_word: string): Promise<boolean> => {
  try {
...
export const fetchSwappableSchedules = async (scheduleId: string): Promise<WorkSchedule[]> => {
  const response = await Axios(SummaryApi.schedules.getSwappableSchedules(scheduleId));
  return response.data.data || [];
};
